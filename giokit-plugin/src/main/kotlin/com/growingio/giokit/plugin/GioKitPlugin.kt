...
import com.growingio.giokit.plugin.transform.v3.GiokitV3Factory
...
class GioKitPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        project.plugins.withType(AndroidBasePlugin::class.java) {
            ...
            androidExtension.forEachRootVariant { variant ->
                GioKitConfigProcessor(project, v3Config).process(variant)
            }
            ...
            if (SimpleAGPVersion.ANDROID_GRADLE_PLUGIN_VERSION < SimpleAGPVersion(4, 2)) {
                ...
                configureBytecodeTransform(project, v3Config)
            } else {
                // Configures bytecode transform using AGP 4.2 ASM pipeline.
                configureBytecodeTransformASM(project, v3Config)
            }
        }
    ...
    private fun configureBytecodeTransform(project: Project, v3Config: GioConfig) {
        ...
        ).invoke(androidExtension, GiokitV3Transform(project, androidExtension, v3Config), emptyArray<Any>())
    }
    ...
    private fun configureBytecodeTransformASM(project: Project, v3Config: GioConfig) {
        fun registerTransform(androidComponent: ComponentCompat) {
            androidComponent.transformClassesWith(
                classVisitorFactoryImplClass = GiokitV3Factory::class.java,
