...
import com.growingio.giokit.plugin.transform.HookInjectorClass.WEBVIEW_EXTRA_SYSTEM
import com.growingio.giokit.plugin.transform.HookInjectorClass.WEBVIEW_EXTRA_UC
import com.growingio.giokit.plugin.transform.HookInjectorClass.WEBVIEW_EXTRA_X5
...
class GioWebViewVisitor(api: Int, ncv: ClassVisitor, context: GioTransformContext) :
    ...
    override fun isInjectClass(hookData: HookInjectorClass.HookData): Boolean {
        ...
        } else if (context.isAssignable(context.className, hookData.targetClassName)) {
    ...
    override fun getTargetHookData(): List<HookInjectorClass.HookData> {
        return if (context.gioConfig.hasGioPlugin) {
            HookInjectorClass.WEBVIEW_EXTRA_CLASSES
        } else {
            HookInjectorClass.WEBVIEW_CLASSES
        }
    }
    ...
    override fun generateMethodInsn(name: String, adviceAdapter: AdviceAdapter) {
        when (name) {
            WEBVIEW_SYSTEM,WEBVIEW_EXTRA_SYSTEM -> createWebViewInsnList(adviceAdapter)
            WEBVIEW_X5, WEBVIEW_EXTRA_X5 -> createX5WebViewInsnList(adviceAdapter)
            WEBVIEW_UC, WEBVIEW_EXTRA_UC -> createUCWebViewInsnList(adviceAdapter)
